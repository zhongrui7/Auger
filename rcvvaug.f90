...
!
      ...
      write (jprt,1105) name
      write (jprt,1107) namorb
      write (jprt,1101) ((kap,kapp,kapp=-lmax-1,kap),kap=-lmax-1,-1) , &
                       & ((kap,kapp,kapp=-lmax-1,-1),                   &
                       & (kap,kapp,kapp=1,kap),kap=1,lmax)
1101 format (/1x,'i2',2x,'i',3x,'e2',4x,'ep',4x,'e',2x,28(i6,i3,'    ')&
            & /)
!
      write (jspect,1106) title
      write (jspect,1105) name
      write (jspect,1102) hws , hwlev , hwc , hwlf
1102 format ('   HWS=',f5.2,' eV','    HWL=',f5.2,' eV','   HWC=',f5.2,&
             &' eV','   HWLF=',f5.2,' eV')
!
      conv = 2.0*pi/a0
      conv = conv*conv
...
!
            read (jmat,*) ii , jj , dum , eep , ee ,                    &
                        ...
                        & lmax)
!
            write (jprt,1103) i , j , efile2(i) , efile(jp) , efile(j) &
                             & ,                                        &
                             ...
                             & lmax)
1103       format (2I3,3F6.2,28D13.5)
!
            if ( ii.NE.i .OR. jj.NE.j .OR. dabs(eep-efile(jp))          &
               &  > tiny .OR. dabs(ee-efile(j)) > tiny ) then
...
!
      ...
      write (jprt,1109) (ikap(iord(i)),ikapp(iord(i)),i=1,4)
      write (jprt,1110) (e2(i)-e2max,ptot(i),(p(i,iord(ii)),ii=1,4),   &
                       & pdiag(i),i=1,ne2)
!
!
! broadened spectra
!
      write (jspect,1107) namorb
      write (jspect,1108) ecore*efac
      write (jspect,1109) (ikap(iord(i)),ikapp(iord(i)),i=1,4)
      write (jspect,1110) (e1(i)-e2maxev,p1tot(i)/snorm,(p1(i,ii)/snorm&
                         & ,ii=1,4),p1diag(i)/snorm,i=iminb,n1)
!
      write (jspect,1104) (ikap(iord(i)),ikapp(iord(i)),i=1,4)
1104 format (4x,'e2',5x,4x,'Total',4(4x,i2,1x,i2),4x,'Diag.')
      write (jspect,1108) (e1(i)-e2maxev,p1tot(i),(p1(i,ii),ii=1,4),   &
                         & p1diag(i),i=iminb,n1)
!
!
      stop ' FORTRAN stop'
...
1106 format (a78)
1107 format (a10)
1108 format (f11.5,6F9.3)
1109 format (4x,'e2',5x,5x,'Total',3x,4(5x,i2,1x,i2,3x),5x,'Diag.')
1110 format (f11.5,6E13.5)
      end
...
! halfwidth of Lor. increases quadr.:
               if ( ilor =2 ) gam = Hwlr*((Ef-E(i))/(Ef-E(1)))        &
                                    & **2/2.0E0
...
!
         ...
         enddo
         do i = 1 , N0
            if ( A0(i) > 0.E0 ) goto 200
         enddo
 200     emin0 = e0(i)
         do i = 1 , N1
            if ( E1(i) > emin0 ) goto 250
         enddo
 250     Mine1 = i
      endif
!
...
!
      do i = Mine1 , nlst
         j = 0
 300     j = j + 1
         if ( e0(j) < E1(i) .AND. j < max ) goto 300
         j = j - 2
         if ( j < 1 ) j = 1
         if ( j > max-3 ) j = max - 3
...
!        endif
         ...
         enddo
         if ( Idoc =1 ) write (Jprt,1101)
1101    format (' non-grid value for lower e boundary')
      endif
      if ( insright =1 ) then
         ehelp(ifr) = E2 - Efile2(1) + Efile(1)
!        if(ehelp(ifr) < =emax)then
!          ifl=ifl-1
!          goto 297
!        endif
         do l = -Km - 1 , Km
         ...
         enddo
         if ( Idoc =1 ) write (Jprt,1102)
1102    format (' non-grid value for upper e boundary')
      endif
      if ( Idoc =1 ) write (Jprt,1103) ifl , ifr
1103 format (' ifl,ifr: ',2I4)
!
               ...
               do lp = -Km - 1 , l
                  if ( lp.NE.0 ) then
                     ...
                     call LAGRANGE(Efile(ifl),chelp(ifl),nf,nint,E(ie), &
                                 & Crs(ie,l,lp),iextra)
                     if ( iextra =1 .AND. Idoc =1 )                 &
                        & write (Jprt,1106) ie , E(ie) ,               &
                        & ((Crs(ie,k,kp),kp=-Km-1,k),k=-Km-1,-1) ,      &
                        & ((Crs(ie,k,kp),kp=-Km-1,-1),                  &
                        & (Crs(ie,k,kp),kp=1,k),k=1,Km)
1106                format (' extrapolation: crs, ie=',i3,' e(ie)=',   &
                           & f10.5/28E13.5)
                  endif
               enddo
            endif
...
! Find 1st point to the right of xval if it exists:
 ...
 100  igr = igr + 1
      ...
      if ( X(igr) < Xval .AND. igr < Ndim ) goto 100
!
...
!
!
 100  READ (in,1115,END=99999) filen
...
!
      READ (in,*) norm
      WRITE (pun,1102) norm
1102 FORMAT (1x,'NORMALIZATION OF VALENCE WF. (0 FOR MT, 1 FOR WS):',  &
            & i4/)
!
! read in identifier of core state
!
      READ (in,1116) (norb1(i),i=1,5)
!
      READ (in,*)
!
...
!
      ...
      ENDDO
!
      WRITE (*,1116) (name(i),i=1,40)
      WRITE (pun,1116) (name(i),i=1,40)
      WRITE (pun,*)
      WRITE (pun,1103) zatom , x0 , dx , nmt , rmt , nws , rws , zeromt
1103 FORMAT (1x,' ZATOM =',t20,i4/1x,'    X0 =',t20,f5.1/1x,'    DX =',&
            & t20,f10.7/1x,'   NMT =',t20,i4/1x,'   RMT =',t20,f10.7/1x,&
             &'   NWS =',t20,i4/1x,'   RWS =',t20,f10.7/1x,'ZEROMT =',  &
            & t20,f5.1//1x,'RADIAL MESH AND POTENTIAL*R'/)
      WRITE (pun,1104) (rs(j),v(j)*rs(j),j=1,nws)
1104 FORMAT (4D20.10)
!
...
!
      IF ( norb(1).NE.norb1(1) .OR. norb(2).NE.norb1(2) .OR. norb(3)    &
         & .NE.norb1(3) ) GOTO 200
!
      WRITE (pun,1105) (norb(i),i=1,5) , ec1 , kap1
1105 FORMAT (//1x,'CORE WAVEFUNCTION'/1x,'ORB. =',t20,5A1/1x,' EC1 =', &
            & t20,f12.6/1x,'KAP1 =',t20,i4//)
      WRITE (pun,1106) (rs(j),gc1(j),fc1(j),j=1,nws)
1106 FORMAT (3D20.10)
!
      WRITE (pun,1107)
1107 FORMAT (//' ****************** END INPUT *******************'//)
!
...
!
      ...
      ne2 = 2*ne - 1
      emin2 = 2.*e0 - ec1
      WRITE (pun,1108) (earr(i),i=1,ne)
1108 FORMAT (1x,'ENERGY PLANE'//(7x,15F7.2)/)
!
      DO i = 1 , ne2
         earr2(i) = emin2 + (i-1)*de
         DO j = 1 , ne
            jp = i + 1 - j
            IF ( jp.GE.1 .AND. jp < =ne ) THEN
               ii = ii + 1
               irange(j,i) = ii
            ELSE
               irange(j,i) = 0
            ENDIF
         ENDDO
         WRITE (pun,1109) earr2(i) , (irange(j,i),j=1,ne)
1109    FORMAT (f7.2,15(2x,i3,2x))
      ENDDO
!
      WRITE (pun,*)
!
...
!
               kout1 = 0
               IF ( nonrel =1 .AND. irange(nei,nei2) =ne*ne )       &
                  & kout1 = 1
!
...
!
                  ...
                  DO kap = -lval - 1 , lval
                     IF ( kap.NE.0 ) THEN
                        WRITE (pun,1117) (kap,kapp,kapp=-lval-1,-1) ,  &
                             & (kap,kapp,kapp=1,lval)
                        WRITE (pun,1118) (sd(kap,kapp),kapp=-lval-1,-1)&
...
!         ********************************
!
      WRITE (mat,1112) (name(i),i=1,40) , (norb1(i),i=1,5) , ec1 , e0 ,&
                      & de , ne , lval , a0
1112 FORMAT (1x,                                                       &
...
!
      WRITE (mat,1113) (((kap,kapp),kapp=-lval-1,kap),kap=-lval-1,-1) ,&
                      & (((kap,kapp),kapp=-lval-1,-1),                  &
                      & ((kap,kapp),kapp=1,kap),kap=1,lval)
1113 FORMAT (1x,'i2',2x,'i',3x,'e2',4x,'ep',4x,'e',2x,28(i6,i3,'    '))
!
      DO i2 = 1 , ne2
         DO i = 1 , ne
            ip = i2 + 1 - i
            IF ( irange(i,i2).NE.0 ) THEN
               WRITE (mat,1114) i2 , i , earr2(i2) , earr(ip) , earr(i)&
                               & , ((crsecs(kap,kapp,i,i2),kapp=-lval-1,&
                               & kap),kap=-lval-1,-1) ,                 &
                               & ((crsecs(kap,kapp,i,i2),kapp=-lval-1,  &
                               & -1),(crsecs(kap,kapp,i,i2),kapp=1,kap),&
                               & kap=1,lval)
1114          FORMAT (2I3,3F6.2,28D13.5)
            ENDIF
         ENDDO
      ENDDO
!
...
!
            WRITE (Pun,1103)
            WRITE (Pun,1104) (kap,kap=-lact-1,-1) , (kap,kap=1,lact)
            WRITE (Pun,1105) (ratx(kap),kap=-lact-1,-1) ,              &
                            & (ratx(kap),kap=1,lact)
            WRITE (Pun,1101)
1101       FORMAT (/1x,'TANGENT PHASESHIFTS'/)
            WRITE (Pun,1104) (kap,kap=-lact-1,-1) , (kap,kap=1,lact)
            WRITE (Pun,1105) (tanx(kap),kap=-lact-1,-1) ,              &
                            & (tanx(kap),kap=1,lact)
!
         ELSE
!
            WRITE (Pun,1103)
            WRITE (Pun,1104) (kap,kap=-1,-lact-1,-1)
            WRITE (Pun,1105) (ratx(kap),kap=-1,-lact-1,-1)
            WRITE (Pun,1106) (kap,kap=1,lact)
            WRITE (Pun,1107) (ratx(kap),kap=1,lact)
            WRITE (Pun,1102)
1102       FORMAT (/1x,'PHASESHIFTS'/)
            WRITE (Pun,1104) (kap,kap=-1,-lact-1,-1)
            WRITE (Pun,1105) (eta(kap),kap=-1,-lact-1,-1)
            WRITE (Pun,1106) (kap,kap=1,lact)
            WRITE (Pun,1107) (eta(kap),kap=1,lact)
!
         ENDIF
!
      ENDIF
1103 FORMAT (/1x,'CF/G RATIO'/)
1104 FORMAT (2x,12(5x,i3,5x))
1105 FORMAT (2x,12D13.5)
1106 FORMAT (15x,11(6x,i2,5x))
1107 FORMAT (15x,11D13.5)
!
      END
!*==COMDIR.spg  
...
!
      IF ( DABS(test*(unp2-unp)) < =DABS(unp2) ) THEN
         IF ( DABS(test*(wnp2-wnp)) < =DABS(wnp2) ) GOTO 500
      ENDIF
!
      IF ( nit.NE.5 ) THEN
         nit = nit + 1
...
!
      ...
      ENDDO
      SINTG = sum/3.D0
      IF ( MOD(N,2) =0 ) SINTG = SINTG + (Fct(N)+Fct(N-1))            &
                                 & /2.D0*(R(N)-R(N-1))
      END
!*==DINTG.spg 
...
!
                           IF ( iwigtes =1 ) THEN
                              WRITE (Pun,1101) kap1 , l1 , j1 , kap2 , &
                                   & l2 , j2 , lam , result
!
1101                         FORMAT (2I4,f5.1,5x,2I4,f5.1,5x,i4,10x,   &
                                    & d17.10)
                           ENDIF
                        ENDIF
!
...
!
               DO lam = 0 , lmax
                  IF ( lam.GE.lami .AND. lam < =lama ) THEN
                     xlam = DFLOAT(lam)
!
...
!
                           IF ( iwigtes =1 ) THEN
                              WRITE (Pun,1101) lamp , J1 , j , lam ,   &
                                   & j2 , j3 , sixcof(lamp-lammi+1)
!
1101                         FORMAT (i4,2F5.1,5x,i4,2F5.1,10x,d17.10)
                           ENDIF
                        ENDDO
                     ENDIF
                  ENDIF
...
!
                        Ier = -2
                        WRITE (9,1101) L2 , L3 , m1 , M2 , M3 , nfin , &
                                      & Ndim
1101                   FORMAT (///1x,'3j-coefficients',9x,'l1',        &
                              & 2F7.1/20x,3F7.1,4x,                     &
...
!
      Ier = -1
      WRITE (9,1102) L2 , L3 , m1 , M2 , M3
1102 FORMAT (///1x,'3j-coefficients',9x,'l1',2F7.1/20x,3F7.1,4x,       &
...
!
                  ...
                  DO i = 1 , lstep
                     index = nfin - i + 1
                     IF ( DABS(Thrcof(index)) < srtiny ) Thrcof(index) &
                        & = zero
...
!
                                    Ier = -2
                                    WRITE (9,1101) L2 , L3 , L4 , L5 , &
                                     & L6 , nfin , Ndim
1101                               FORMAT (///1x,'6j-coefficients',9x, &
...
!
      Ier = -1
      WRITE (9,1102) L2 , L3 , L4 , L5 , L6
1102 FORMAT (///1x,'6j-coefficients',9x,'l1',2F7.1,4x,                 &
...
!
                  ...
                  DO i = 1 , lstep
                     index = nfin - i + 1
                     IF ( DABS(Sixcof(index)) < srtiny ) Sixcof(index) &
                        & = zero
